...
const cur = (n: number) =>
  new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR", maximumFractionDigits: 1 }).format(
    isFinite(n) ? n : 0
  );
...
export default function ProperRentalProfitabilityPlanner() {
  ...
  const totals = useMemo(() => {
    const data = apartments.map((a) => {
      ...
      return { ...a, grossAnnualRent, operatingCost, netOperatingIncome, annualLoan, beforeTaxCashFlow, incomeTaxBaseAmount, incomeTaxAmount, cashFlow, roi5, roi10, roi15 };
    });
    ...
    <div className="min-h-screen w-full bg-white p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-col gap-2">
          ...
          <p className="text-slate-600">Ce tableau inclut l'impôt sur le revenu locatif. Vous pouvez choisir son assiette : <strong>Brut annuel</strong> (votre demande) ou <strong>Cashflow avant impôt</strong>.</p>
        ...
        <Button variant="secondary" onClick={addApartment}><Plus className="h-4 w-4 mr-1"/>Ajouter un appartement</Button>
            ...
            <tbody>
              {totals.data.map((a) => (
                <tr key={a.id} className="border-t">
                  ...
                  <td className="p-2 text-center"><Input type="number" value={a.monthlyRate} onChange={(e) => updateApartment(a.id, { monthlyRate: parseFloat(e.target.value || "0") })} className="w-24 text-center" /></td>
                  <td className="p-2 text-center"><Input type="number" value={a.occupancy} onChange={(e) => updateApartment(a.id, { occupancy: parseFloat(e.target.value || "0") })} className="w-16 text-center" /></td>
                  <td className="p-2 text-center"><Input type="number" value={a.operatingExpenses} onChange={(e) => updateApartment(a.id, { operatingExpenses: parseFloat(e.target.value || "0") })} className="w-16 text-center" /></td>
                  <td className="p-2 text-center"><Input type="number" value={a.propertyTax} onChange={(e) => updateApartment(a.id, { propertyTax: parseFloat(e.target.value || "0") })} className="w-16 text-center" /></td>
                  <td className="p-2 text-center"><Input type="number" value={a.incomeTax} onChange={(e) => updateApartment(a.id, { incomeTax: parseFloat(e.target.value || "0") })} className="w-16 text-center" /></td>
                  <td className="p-2 text-center"><Input type="number" value={a.loanMonthly} onChange={(e) => updateApartment(a.id, { loanMonthly: parseFloat(e.target.value || "0") })} className="w-24 text-center" /></td>
                  ...
                  <td className="p-2 text-center"><Button variant="ghost" size="icon" onClick={() => removeApartment(a.id)}><Trash2 className="h-4 w-4"/></Button></td>
                </tr>
              ))}
            </tbody>
        ...
        <p className="text-xs text-slate-500 italic">
          Méthodologie professionnelle : le Revenu Brut Potentiel est réduit par les frais d’exploitation et la taxe foncière pour obtenir le NOI (Net Operating Income). Le remboursement annuel du prêt est ensuite déduit pour obtenir le cashflow avant impôt. L’impôt sur le revenu locatif est ensuite appliqué pour calculer le cashflow net après impôt. Les ROI sont calculés sur base du cashflow net.
        </p>
      </div>
    </div>
  );
